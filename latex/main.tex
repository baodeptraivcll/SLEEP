...
\author{
\IEEEauthorblockN{
1\textsuperscript{st} Nguyễn Viết Hưng,
2\textsuperscript{nd} Lê Đức Gia Bảo,
3\textsuperscript{rd} Huỳnh Quang Trí, and
4\textsuperscript{th} Vũ Nguyễn Minh Long
}
\IEEEauthorblockA{
...
Convolutional recurrent architectures have played a major role in this development. DeepSleepNet combines parallel convolutional branches with bidirectional recurrent modeling \cite{supratak2017deepsleepnet}, while TinySleepNet uses a more compact CNN--LSTM design \cite{supratak2020tinysleepnet}. Sequence-to-sequence methods such as SeqSleepNet explicitly predict all stages in an input context \cite{phan2019seqsleepnet}. Other studies have explored joint classification and neighboring-stage prediction \cite{phan2019joint}, fully convolutional segmentation \cite{perslev2019utime}, and hybrid CNN--LSTM architectures \cite{li2023hybrid,toma2023multichannel}.
...
Every model is trained for at most 50 epochs with batch size 32. AdamW is used with learning rate $10^{-4}$ and weight decay $10^{-4}$ \cite{loshchilov2019adamw}. Gradients are clipped to a maximum norm of 5.0. Python, NumPy, and PyTorch random generators use seed 42, with deterministic cuDNN behavior enabled.
...
Fig.~\ref{fig:learning_curves} illustrates the aggregated learning curves for the four evaluated architectures. TinySleepNet-adapted and SleepTransformer-adapted converged rapidly, reaching stable performance near $0.74$ within 20 epochs on validation macro-F1. DeepSleepNet-adapted exhibited slow convergence and substantial variation across different subject-wise validation folds, which reflects the difficulty of training the deeper dual-branch CNN--BiLSTM architecture in a single end-to-end stage without two-stage optimization. SleepTransformer-adapted and MambaSleep-adapted achieved lower final training losses (below $0.10$) compared to TinySleepNet-adapted and DeepSleepNet-adapted (approximately $0.20$), indicating higher capacity. MambaSleep-adapted converged to a slightly lower validation macro-F1 ($0.70$) but exhibited very low variation across folds (indicated by the narrow standard deviation band), which is consistent with its lightweight state-space design.

In terms of computational efficiency, TinySleepNet-adapted required $105.8 \pm 10.8$ minutes per fold on average ($134.0 \pm 3.0$ s/epoch, averaging $47.4$ epochs). DeepSleepNet-adapted and SleepTransformer-adapted required the most training time, taking $127.5 \pm 3.2$ minutes ($150.6 \pm 3.5$ s/epoch, averaging $50.8$ epochs) and $129.3 \pm 15.7$ minutes ($165.1 \pm 2.0$ s/epoch, averaging $47.0$ epochs) per fold, respectively. MambaSleep-adapted was the fastest architecture to train, requiring $83.4 \pm 17.1$ minutes per fold on average ($121.8 \pm 2.8$ s/epoch, averaging $41.0$ epochs). These durations indicate that the lightweight state-space layers in MambaSleep-adapted offer noticeable speed advantages over recurrent and self-attention operations in this sequence staging pipeline.
...
& Our adapted implementation & Sleep-EDF (78 sub.), Fpz--Cz & Subject-wise 10-fold (20-epoch) & 87.61\% ACC, 70.91\% MF1  \\
\bottomrule
\end{tabular}
\end{table*}
