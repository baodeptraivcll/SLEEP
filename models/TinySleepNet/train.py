...
def run_training(model, train_loader, val_loader, test_loader, train_files, args, device):
    ...
    np.save(os.path.join(out_dir, "confusion_matrix.npy"), np.array(test_metrics["confusion_matrix"]))
    np.savez(os.path.join(out_dir, "predictions.npz"), y_true=test_metrics["y_true"], y_pred=test_metrics["y_pred"])
