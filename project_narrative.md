...
## 2. Dàn Diễn Viên (4 Models & Vai trò)
...
### 3. SleepTransformer - "Gã Khổng Lồ Toàn Cảnh"
...
### 4. WaveMamba (State Space Model) - "Kẻ Thách Thức Tối Thượng"
...
- 4 người tự code 4 kiến trúc lõi (DeepSleepNet, TinySleepNet, Transformer, Mamba) bằng PyTorch.
...
- Ném tất cả kết quả của 4 người vào 1 đoạn code Python chung để vẽ biểu đồ.
- Sinh ra: Biểu đồ cột so sánh F1-Score, Bảng Confusion Matrix chéo, và Đỉnh cao nhất là **Biểu đồ Hypnogram** đè 4 model lên sóng não thực tế.
...
- **Bắt buộc xài WandB (Weights & Biases):** Mọi quá trình Train của 4 người phải được log lên hệ thống WandB để minh chứng thời gian thực hiện trong học kỳ.
  - *Luật chuẩn hóa tên biến WandB:* Để vẽ gộp được 4 mô hình lên cùng 1 đồ thị, cả 4 người PHẢI dùng chung chính xác các keys sau trong hàm `wandb.log()`: `"epoch"`, `"train_loss"`, `"val_f1"`, `"val_kappa"`, `"val_acc"`, `"final_fold_acc"`, `"final_fold_f1"`, `"final_fold_kappa"`.
...
- **Present 2 (Báo cáo cuối kỳ):** Hoàn thành full paper, full biểu đồ so sánh 4 models.
...
1. **Introduction:** Tổng quan bài toán Sleep Staging, tại sao chọn (khó khăn thực tế), nhóm đã giải quyết gì (đánh giá đa chiều 4 models), đóng góp của nhóm.
2. **Related works:** Tóm tắt 30+ bài báo. Nêu bật ưu/khuyết điểm của các phương pháp cũ (RNN/CNN) và mới (Attention/SSM).
3. **Methodology:** Trình bày chi tiết phương pháp, luồng DataLoader chung, và kiến trúc của 4 model tham gia thi đấu.
4. **Experimental and Results:** 
   ...
   - **Kết quả:** Bảng so sánh 4 models. Thảo luận (Discussion) điểm mạnh/yếu từng thằng. **Ablation Study** (bắt buộc cho TinySleepNet).
